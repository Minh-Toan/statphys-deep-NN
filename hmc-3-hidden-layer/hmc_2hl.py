...
def data_generate(d, alpha, gamma_1, gamma_2, sig_1, sig_2, Delta, vlaw):
    ...
    if vlaw == 'gaussd':
        v = tf.reduce_mean(tf.sort(tf.random.normal((10000, k2))), axis=0)
        v = v * np.sqrt(k2) / tf.norm(v)
    if vlaw == 'radd':
        l = int(k2/2)
...
def hmc(params, W10, W20, v, X, Y, Delta, sig_1, sig_2, info):
    k1, d = W10.shape
    k2, _ = W20.shape
    ...
    if info:
        W_init = tf.concat([tf.reshape(W10, [-1]), tf.reshape(W20, [-1])], axis=0)
    else:
        W_init = tf.random.normal((k1*d + k2*k1,), dtype=tf.float32)
